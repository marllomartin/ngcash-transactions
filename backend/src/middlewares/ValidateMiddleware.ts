  ...
  static async validateUser(req: Request, res: Response, next: NextFunction) {
    ...
    if (username.length < 3) {
      return res.status(StatusCodes.BAD_REQUEST)
        .json({ message: 'Username must be at least three characters long' });
    }

    if (password.length < 8) {
      return res.status(StatusCodes.BAD_REQUEST)
        .json({ message: 'Password must be at least eight characters long' });
    }

    if (password.search(/[A-Z]/) < 0) {
      return res.status(StatusCodes.BAD_REQUEST)
        .json({ message: 'Password needs at least one uppercase letter' });
    }
  ...
  static async validateTransaction(req: Request, res: Response, next: NextFunction) {
    ...
    if (debitedAccountId === creditedAccountId) {
      return res.status(StatusCodes.FORBIDDEN)
        .json({ message: 'Invalid transaction: cannot send funds to same account' });
    }
